...
`);
	...
	describe('.prototype.children()', function() {
		it('Selects child nodes.', function() {
			expect(pq.children().getNodes()).toEqual(sampleData[4].children.concat(sampleData[5].children));
		});
		it('Filters child nodes by name.', function() {
			expect(pq.children('bargle').getNodes()).toEqual([sampleData[4].children[1]]);
		});
		it('Filters child nodes by value.', function() {
			expect(pq.children(null, '').getNodes()).toEqual([sampleData[5].children[0], sampleData[5].children[1]]);
		});
	});
	describe('.prototype.getLength()', function() {
		it('Returns the root count.', function() {
			expect(pq.getLength()).toEqual(6);
		});
		it('Returns the joined child count.', function() {
			expect(pq.children().getLength()).toEqual(5);
		});
	});
	describe('.prototype.getNodes()', function() {
		it('Returns the root array.', function() {
			expect(pq.getNodes()).toEqual(sampleData);
		});
	});
	describe('.prototype.getName()', function() {
		it('Returns the name of the last node.', function() {
			expect(pq.getName()).toEqual('extra');
		});
		it('Returns the name of the last node from the selection.', function() {
			expect(pq.children().getName()).toEqual('2');
		});
	});
...
`));
		it('Selects descendant nodes.', function() {
			expect(pq.descendants().getNames()).toEqual('bcefgh'.split(''));
		});
		it('Filters descendant nodes by name.', function() {
			expect(pq.descendants('e').getNodes()).toEqual([d[1].children[0]]);
		});
		it('Filters descendant nodes by value.', function() {
			expect(pq.descendants(null, '').getNames()).toEqual('bcfh'.split(''));
		});
	});
...
`));
		...
		it('Filters parent node by name.', function() {
			expect(pq.descendants(null, '1').parent('c').getNodes()).toEqual([d[0].children[1]]);
		});
	});
...
`));
		...
		it('Filters parent nodes by name.', function() {
			expect(pq.descendants('e').parents('c').getNodes()).toEqual([d[0].children[1]]);
		});
	});
...
`));
		it('Selects closest matching node.', function() {
			expect(pq.descendants(null, '1').closest('a').getNodes()).toEqual([d[0].children[1].children[0].children[0], d[0]]);
		});
	});
...
`));
		it('Selects the node at the specified index.', function() {
			expect(pq.descendants().byIndex(1).getNodes()).toEqual([d[0].children[0].children[0]]);
		});
	});
...
`));
		it('Selects the first node.', function() {
			expect(pq.descendants().first().getNodes()).toEqual([d[0].children[0]]);
		});
	});
...
`));
		it('Selects the last node.', function() {
			expect(pq.descendants().last().getNodes()).toEqual([d[0].children[1]]);
		});
	});
...
`));
		it('Selects the root node.', function() {
			expect(pq.descendants('c').root().getNodes()).toEqual([d[0]]);
		});
	});
...
`));
		...
		it('Filters previous siblings by name.', function() {
			expect(pq.descendants('g').previousAll('e').getNodes()).toEqual([d[1].children[2]]);
		});
		it('Filters previous siblings by value.', function() {
			expect(pq.descendants('g').previousAll(null, '').getNodes()).toEqual([d[1].children[1], d[1].children[0]]);
		});
	});
...
`));
		it('Selects previous sibling.', function() {
			expect(pq.descendants(null, '1').previous().getNodes()).toEqual([d[0].children[1].children[0], d[0].children[1].children[2], d[0].children[1]]);
		});
		it('Filters previous sibling by name.', function() {
			expect(pq.descendants(null, '1').previous('e').getNodes()).toEqual([d[0].children[1].children[2]]);
		});
		it('Filters previous sibling by value.', function() {
			expect(pq.descendants(null, '1').previous(null, '').getNodes()).toEqual([d[0].children[1].children[0]]);
		});
	});
...
`));
		...
		it('Filters next siblings by name.', function() {
			expect(pq.descendants('d').nextAll('e').getNodes()).toEqual([d[0].children[1].children[2]]);
		});
		it('Filters next siblings by value.', function() {
			expect(pq.descendants('d').nextAll(null, '').getNodes()).toEqual([d[0].children[1].children[3], d[0].children[1].children[4]]);
		});
	});
...
`));
		it('Selects next sibling.', function() {
			expect(pq.descendants(null, '1').next().getNodes()).toEqual([d[0].children[1], d[0].children[1].children[2], d[0].children[1].children[4]]);
		});
		it('Filters next sibling by name.', function() {
			expect(pq.descendants(null, '1').next('e').getNodes()).toEqual([d[0].children[1].children[2]]);
		});
		it('Filters next sibling by value.', function() {
			expect(pq.descendants(null, '1').next(null, '').getNodes()).toEqual([d[0].children[1].children[4]]);
		});
	});
...
`));
		...
		it('Fixes next sibling links in remaining nodes.', function() {
			expect(pq.descendants('b').getNode(0).nextSibling).toEqual(d[1]);
		});
		...
		it('Fixes previous sibling links in remaining nodes.', function() {
			expect(pq.descendants('e').getNode(0).previousSibling).toEqual(d[0]);
		});
